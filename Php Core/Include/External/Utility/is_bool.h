	...
	namespace PHP {
		...
		PHP_FUNCTION(is_bool) {
			if (arguments.length() != 1) {
				throw std::runtime_error("is_bool takes exactly one argument!");
			}
			PHP_RETURN(Value(arguments[0].value()->type() == ValueType::BOOLEAN_TYPE));
		}

		PHP_FUNCTION_SIGNATURE(is_bool) {
