	...
	namespace PHP {
		...
		PHP_FUNCTION(is_array) {
			if (arguments.length() != 1) {
				throw std::runtime_error("is_array takes exactly one argument!");
			}
			PHP_RETURN(Value(arguments[0].value()->type() == ValueType::ARRAY_TYPE));
		}

		PHP_FUNCTION_SIGNATURE(is_array) {
